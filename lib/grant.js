...
function extractCredentials (done) {
  ...
  var type = is.is(contentType, ['*/*+json', 'x-www-form-urlencoded']);
  var vre = /docler.access-token.v1/;
  var version = vre.exec(contentType);

  if (!type) {
    return done(error('invalid_request',
      'Only content type of application/x-www-form-urlencoded or application/vnd.docler.access-token.v1+json encoding are allowed'));
  }

  if (!version) {
    return done(error('invalid_version', 'Invalid API version: expected "docler.access-token.v1"'));
  }
...
function useRefreshTokenGrant (done) {
  ...
  this.model.getRefreshToken(token, function (err, refreshToken) {
    ...
    if (!refreshToken || refreshToken.clientId !== self.client.clientId) {
      return done(error('invalid_token', 'Invalid refresh token'));
    } else if (refreshToken.expires !== null &&
        refreshToken.expires < self.now) {
      return done(error('invalid_token', 'Refresh token has expired'));
    }
