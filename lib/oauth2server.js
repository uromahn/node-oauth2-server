...
function OAuth2Server (config) {
  ...
  this.acceptedContentType = config.acceptedContentType || ['application/x-www-form-urlencoded', 'application/json'];
  this.apiVersion = config.apiVersion;

  this.regex = {
    clientId: config.clientIdRegex || /^[a-z0-9-_]{3,40}$/i,
    grantType: new RegExp('^(' + this.grants.join('|') + ')$', 'i'),
    vre: new RegExp(config.apiVersion) || /.*/
  };

  this.returnUserInfo = config.returnUserInfo !== undefined ? config.returnUserInfo : false;
}
